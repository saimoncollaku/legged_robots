...
P3D GeneralizedCoordinatesRobotRepresentation::
    ...
    // Fetching quantities of interest 
    V3D vChild = V3D(getJointForQIdx(qIndex)->cJPos, pLocal);
    V3D rotAxis = getQAxis(qIndex);
    double rotAngle = getQVal(qIndex);
    
    // Getting the point in parent frame
    V3D vParent = rotateVec(vChild, rotAngle, rotAxis);
    P3D pParent = getJointForQIdx(qIndex)->pJPos + vParent;
...
P3D GeneralizedCoordinatesRobotRepresentation::getWorldCoordinates(const P3D &p,
    ...
    // Here we spit out the point in base frame (last currentP)
    P3D currentP = p;
    RB *currentRB = rb;
    int qIdx = getQIdxForJoint(currentRB->pJoint);
    while (qIdx > 5) // first 6 are base DoFs
    {
        currentP = getCoordsInParentQIdxFrameAfterRotation(qIdx, currentP);
        currentRB = currentRB->pJoint->parent;
        qIdx = getQIdxForJoint(currentRB->pJoint);
    }
    
    // Combining base pos/orient with child vars
    P3D pBase = P3D(getQVal(0), getQVal(1), getQVal(2));
    Quaternion rootRotation = getOrientationFor(robot->root);
    pInWorld = pBase + rootRotation * V3D(currentP);
...
void GeneralizedCoordinatesRobotRepresentation::compute_dpdq(const P3D &p,
    ...
    P3D pWorld = getWorldCoordinates(p, rb);
    
    // translation dof
    for (int i = 0; i < 3; i++) 
    {
        dpdq(0, i) = getQAxis(i)[0];
        dpdq(1, i) = getQAxis(i)[1];
        dpdq(2, i) = getQAxis(i)[2];
    }
    ...
    for (int i = 3; i < (int)q.size(); i++) 
    {
        ...
        dpdq(1, i) = 0;
        dpdq(2, i) = 0;
        RBJoint *joint = getJointForQIdx(i);
        
        // Check if dof affects current rb
        bool affectsRB = false;
        RB *currentRB = rb;
        while (currentRB != nullptr) {
            if (currentRB->pJoint == joint) {
                affectsRB = true;
                break;
            }
            if (currentRB->pJoint == nullptr) break;
            currentRB = currentRB->pJoint->parent;
        }
        if (!affectsRB) continue;
        
        // Calculate the effect of this dof on the point position
        V3D axisWorld = getWorldCoordsAxisForQ(i);
        
        // pos of the rotation axis origin in world frame
        P3D jointPosWorld;
        if (joint != nullptr) {
            RB* parentRB = joint->parent;
            jointPosWorld = getWorldCoordinates(joint->pJPos, parentRB);
        } else {
            // For root orientation dof
            jointPosWorld = P3D(q[0], q[1], q[2]);
        }
        
        // change due to rotation around the axis
        // dp/dq = axis × (p - jointPos)
        V3D r = V3D(pWorld) - V3D(jointPosWorld);
        V3D dpDqi = axisWorld.cross(r);
...
void GeneralizedCoordinatesRobotRepresentation::estimate_linear_jacobian(
    ...
    for (int i = 0; i < q.size(); i++) {
        ...
        V3D dpdq_i = (V3D(p_p) - V3D(p_m)) / (2.0 * h);
