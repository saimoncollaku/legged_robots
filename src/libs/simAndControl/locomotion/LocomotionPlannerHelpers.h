...
private:
    ...
    // * NEW EX 4
    double groundHeightForBFrame = 0.0;
    ...
    void generate(const bFrameState& startingbFrameState) {
        ...
        while (t < tEnd) {
            ...
            pos.y = targetbFrameHeight + groundHeightForBFrame ; // * NEW EX 4
            ...
            Quaternion headingQuat = Quaternion(AngleAxisd(headingAngle, RBGlobals::worldUp));
            V3D forwardDir = headingQuat * robot->forward;
            V3D sidewaysDir = headingQuat * RBGlobals::worldUp.cross(robot->forward);

            pos = pos + forwardDir * vForward * dt;
            pos = pos + sidewaysDir * vSideways * dt;
...
public:
    ...
    // * NEW EX 4
    void setGroundHeightForBFrame(double gh) {
        groundHeightForBFrame = gh;
    }
